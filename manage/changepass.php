...
if($_POST['userpass']!=""){
	...
	$newpasswd=trim($_POST['newpass']);
	$userpass=trim($_POST['userpass']);

	$row= $DB_site->query_first("select loginid, user_password from au_admin where loginid='".$_SESSION['loginid']."' and user_status='1'");
	if ($row['loginid']>0){
		//$storedPWD = $row['user_password'];
       // $salt = substr($storedPWD, 0, 12 );
        //$encryptedPWD = crypt($userpass, $salt);

		if( $userpass == decyrptPassword($row['user_password'])){
			$passwd = encyrptPassword($newpasswd);

			$DB_site->query("update au_admin set user_password='$passwd' where loginid='".$_SESSION['loginid']."'");
...
<body class=" theme-blue">
  ...
  <!--<![endif]-->
    ...
    <div class="panel panel-default">
        <p class="panel-heading no-collapse">Change Password</p>
        <div class="panel-body">
            <form name="frm" method="post" onsubmit="return validate()">
                ...
                <label>New Password</label>
                    <input type="password" class="form-controlspan12 form-control" name="newpass">
                </div>
				<div class="form-group">
                <label>Confirm Password</label>
                    <input type="password" class="form-controlspan12 form-control" name="confpass">
                </div>
	...
	<p><a href="index.php">Return to Admin Section</a></p>
</div>
...
function validate(){
	...
	if(document.frm.userpass.value==""){
		...
		return false;
	}else if(document.frm.newpass.value!=document.frm.confpass.value){
		alert("New & Confirm Password should be same");
		document.frm.confpass.focus();
		return false;
	}
}
