...
if($_GET['dl']=="xls" && checkuseraccess(0,0,5)){
	...
	header("Content-disposition:attachment;filename=repoordcustomer.xls"); 
	header("Expires: 0");
...
<div class="btn-toolbar list-toolbar">
	<form method="get" name="frm" action="repoordcustomer.php">
...
<table class="table table-striped" <?php echo ($_GET['dl']=="xls" ? 'border="1"' : '')?>>
  ...
  <tbody>
	<?php 
	$i=0; 
	$sql="SELECT sum(ordtotal) as cnt, concat(c.username,' ',c.lastname) as user, concat(o.username,' ',o.lastname) as ouser, o.email, o.phone FROM `ccd9orders` o join ccd9company c on c.email=o.email where o.status>0  ";
	...
	$sql=$sql." group by o.email order by cnt desc";
