...
<table width="650" border="1" cellpadding="5" cellspacing="0" class="table order-table">
	...
	<tbody>
		...
		while($rescart=$result->fetch_array()){
		 ...
		 $ordpg=$ordpg.'<tr>';
		 if($filename=="order.php"){
			 $ordpg=$ordpg.'<td class="hidden-print"><a href="orderchart.php?orderid='.$orderid.'&prodid='.$rescart['prodid'].'&q='.$n.'&t='.$num_rows.'" target="_blank"><i class="fa fa-pencil"></i></a></td>';
		 }
