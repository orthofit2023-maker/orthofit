...
<div class="row">';
	...
	$ordpg=$ordpg.'<div class="col-md-9">';
					...
					$ordpg=$ordpg.'<div class="row pb20 pt20">
						...
						<div class="col-md-8 col-xs-6">
							...
							<BR><small>Model: '.$roword['prodcode'].' | Color: '.$roword['prodcolor'].' | Qty: '.$roword['prodqty'].'</small>
							'.($roword['measlist']!='' ? '<BR><small>'.getmeas($roword['measlist']).'</small>' : '').'
						</div>
						<div class="col-md-2 col-xs-3 pt5">
						'.$showcur.(($roword['finalprice']*$roword['prodqty'])+ ($roword['shipprice']*$roword['prodqty'])).'
						</div>
					</div>';
				}
	...
	<div class="col-md-3">
		<div class="row pb20">
			<h4 class="box-heading pb10 col-md-12"><div class="pl15">Order Summary</div></h4>';
			if($num_rows>0){
				if($discount>0 || $shiptot>0){
			$ordpg=$ordpg.'<div class="col-md-6 col-xs-6">Sub Total</div>
			<div class="col-md-6 col-xs-6 pull-right">'.$showcur.$tot.'</div>';
				}
			if($discount>0){
			$ordpg=$ordpg.'<div class="col-md-6 col-xs-6">Discount</div>
			<div class="col-md-6 col-xs-6 pull-right">'.$showcur.$discount.'</div>';
			} 
			if($shiptot>0){
			$ordpg=$ordpg.'<div class="col-md-6 col-xs-6">Shipping</div>
			<div class="col-md-6 col-xs-6 pull-right">'.$showcur.$shiptot.'</div>';
			}
			$ordpg=$ordpg.'<div class="col-md-6 col-xs-6">Total Payable</div>
			<div class="col-md-6 col-xs-6 pull-right">'.$showcur.($tot+$shiptot-$discount).'</div>';
