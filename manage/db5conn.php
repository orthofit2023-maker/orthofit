		...
		if($fieldval!=''){
			...
			if($dbname=="ccd9prod2cat" || $dbname=="ccd9prod2type1"  || $dbname=="ccd9prod2type2" || $dbname=="ccd9prod2type3"){
				if($dbname=="ccd9prod2cat"){
					$mysqli->query("insert into ccd9prod2cat (prodid, catid) values ('$id', '$fieldval')");
				}else if($dbname=="ccd9prod2type2"){
					$querysql="select typeid from ccd9types where typename='$fieldval' and opt='$opt'";
					$result = $mysqli->query($querysql);
					if($rescon = $result->fetch_array()){
						$fieldid = $rescon['typeid'];
						$mysqli->query("insert into $dbname (prodid, typeid) values ('$id', '$fieldid')");
					}
					
				}else{
					$mysqli->query("insert into $dbname (prodid, typeid) values ('$id', '$fieldval')");
...
function getauthormenu($id){
	$retval=$retval.'<option value="0">Select Author</option>';

	$db = Database::getInstance();
	$mysqli = $db->getConnection(); 
	$sql="select typeid, typevalue, typename from ccd9types where opt='8' order by typeid";
	$result = $mysqli->query($sql);
...
function getcolors($id,$produrl){
	...
	$db = Database::getInstance();
	$mysqli = $db->getConnection(); 
	$sql="select t3c.typeid,t3c.typename, t3c.typevalue from ccd9prod2type3 t3 join ccd9types t3c on t3.typeid=t3c.typeid and t3c.opt=7  where t3.prodid='$id' and  t3c.typename!='NA' and t3.status='1' order by t3c.typeid";
	$result = $mysqli->query($sql);
	while($rescon = $result->fetch_array()){$retsel="";
...
function gettype1menu($id=0, $tid=0){
	...
	$db = Database::getInstance();
	$mysqli = $db->getConnection(); 
	$sql="select t.typeid, t.typename, ifnull(p.typeid,0) as type1 from ccd9types t left join ccd9prod2type1 p on p.typeid=t.typeid and p.prodid='$id' where t.opt='3' order by t.typename";
	$result = $mysqli->query($sql);
	while($rescon = $result->fetch_array()){$retsel="";
		if($rescon['type1']>0 || ($rescon['typename']==$tid && $tid>0)){
			$retsel=" selected";
		}
		$retval=$retval.'<option value="'.$rescon['typeid'].'" '.$retsel.'>'.$rescon['typename'].'</option>';
	} 
...
function gettype2menu($id=0, $tid=0){
	...
	$db = Database::getInstance();
	$mysqli = $db->getConnection(); 
	$sql="select t.typeid, t.typename, ifnull(p.typeid,0) as type2 from ccd9types t left join ccd9prod2type2 p on p.typeid=t.typeid and p.prodid='$id' where t.opt='4' order by t.typename";
	$result = $mysqli->query($sql);
	while($rescon = $result->fetch_array()){
		$retsel="";
		if($rescon['type2']>0 || ($rescon['typename']==$tid && $tid>0)){
			$retsel=" selected";
		}
		$retval=$retval.'<option value="'.$rescon['typename'].'" '.$retsel.'>'.$rescon['typename'].'</option>';
	} 
	return $retval;
}
...
function gettype3menu($id=0, $tid=0){
	...
	$db = Database::getInstance();
	$mysqli = $db->getConnection(); 
	$sql="select t.typeid, t.typename, ifnull(p.typeid,0) as type3 from ccd9types t left join ccd9prod2type3 p on p.typeid=t.typeid and p.prodid='$id' where t.opt='7' order by t.typename";
	$result = $mysqli->query($sql);
	while($rescon = $result->fetch_array()){$retsel="";
		if($rescon['type3']>0 || ($rescon['typename']==$tid && $tid>0)){
			$retsel=" selected";
		}
		$retval=$retval.'<option value="'.$rescon['typeid'].'" '.$retsel.'>'.$rescon['typename'].'</option>';
	} 
