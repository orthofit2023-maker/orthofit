...
if($_POST['session']!="" && checkuserref()){
	...
	while(($linearray = fgetcsv($file, 5000, ",")) !== false){
		...
		if($inputyes==1 && $linearray[0]!='' && $linearray[0]!='Code'){
			...
			if(trim($linearray[39])=='All')$linearray[39]='ALL'; //zone
			if(trim($linearray[10])=='')$linearray[10]='NA'; //sizes
			if(trim($linearray[41])!='')$linearray[41]=strtoupper($linearray[41]);


			if(trim($linearray[33])=='Yes' || trim($linearray[33])=='YES' || trim($linearray[33])==''){
				$linearray[33]=1;
			}else{
				$linearray[33]=0;
			}
			if(trim($linearray[24])=='Yes' || trim($linearray[24])=='YES' || trim($linearray[24])==''){
				$linearray[24]=1;
			}else{
				$linearray[24]=0;
			}
			...
			if($prodid>0){
				//--------------product categories----------------------------
				storearray(trim($linearray[3]), $prodid, 'ccd9prod2cat','2');

				//--------------product type 1----------------------------
				storearray(trim($linearray[4]), $prodid, 'ccd9prod2type1','3');

				//--------------product type 2----------------------------
				storearray(trim($linearray[5]), $prodid, 'ccd9prod2type2','4');
				...
				storearray(trim($linearray[42]), $prodid, 'ccd9prod2type9', '9');

				//--------------product type 8 whomtype-------------------------
				storearray(trim($linearray[43]), $prodid, 'ccd9prod2type8', '8');

				//--------------product type 10 occasion1-----------------------
				storearray(trim($linearray[44]), $prodid, 'ccd9prod2type10', '10');

				//--------------product type 11 occasion2-----------------------
				storearray(trim($linearray[45]), $prodid, 'ccd9prod2type11', '11');
	...
	while($row = $result->fetch_array()){
		for($n=1;$n<20;$n++){	
			...
			$file1 =$imgcodepath.dbval($row['prodcode']).strtolower(chr($n+64)).'.jpg';
			$newfile =$imgpath.dbval($row['produrl']).'-'.strtolower(chr($n+64)).'0.jpg';
			if(file_exists($file1) && !file_exists($file) && !file_exists($newfile)){
				copy($file1, $newfile);
				rename($file1, $file);
			}
	...
	$arrsize=array();
	$sql="SELECT prodsize FROM ccd9products where prodid in (select prodid from ccd9prod2cat where catid='148') and prodstatus='1' group by prodsize";
	$result = $mysqli->query($sql);
	while($row = $result->fetch_array()){
		$str=dbval($row['prodsize']);
		if(strstr($str,',')){
			$prodarr=explode(',', $str);
		}else{
			$prodarr[0]=trim($str);
	...
	if(count($arrsize)>0){
		...
		//echo "update ccd9types set typename='".implode(",",$arrsize)."' where opt='99' ";
	}
	...
	$mysqli->query("delete FROM ccd9prod2type8 where prodid not in (select prodid from ccd9products ) ");
	$mysqli->query("delete FROM ccd9prod2type9 where prodid not in (select prodid from ccd9products ) ");
	$mysqli->query("delete FROM ccd9prod2type10 where prodid not in (select prodid from ccd9products ) ");
	$mysqli->query("delete FROM ccd9prod2type11 where prodid not in (select prodid from ccd9products ) ");


	$msg= "Added ".$cnti." and ";
	$msg= $msg." Updated ".$cntu." ".$cntup." Products";
