...
<div class="header">
	...
	<li class="active">Abandoned Carts</li>
	</ul>
...
<table class="table table-striped" <?php echo ($_GET['dl']=="xls" ? 'border="1"' : '')?>>
  ...
  <tbody>
	...
	*/
	...
	$sql="SELECT c.prodname, p.prodcode, c.finalprice, c.prodcur, m.city, m.state, cm.countryname, concat(m.username,' ',m.lastname) as user, m.email, m.phone, date_format(c.datemodified,'%d/%b/%y') as cdt FROM `ccd9cart`  c
	join ccd9products p on p.prodid=c.prodid 
	left join ccd9address m on m.compid=c.compid join ccd9country cm on m.countryid=cm.countryid
	where c.orderid=0 and c.compid not in (2,26,3798,7,11,3,93,96,97,98,99,100,106,536, 3760,3630,1395,1311,700,845,50,90,164,566) and m.email!='' ";
	...
	?>
	<tr>
      <td colspan='9'><?php if($num_rows>$g[5]){ echo $g[1];}?></td>
	</tr>
	<?php } }?>
  </tbody>
</table>
