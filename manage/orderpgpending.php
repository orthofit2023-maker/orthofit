...
<table border="1" cellpadding="5" cellspacing="0" class="table order-table">
	<tr><thead>';
		...
		list($prodphoto1, $prodphoto2) = getprodphotos(trim($rescart['photo']), trim($rescart['images']));
		 ...
		 $ordpg=$ordpg.'<br><small>'.($rescart['vprodmeas']!='NA' ? '<b>Fit</b>: '.$rescart['vprodmeas'] : '' ).(trim($rescart['vprodcolor'])!='NA' ? ' <b>Color</b>: '.$rescart['vprodcolor'] : '' ).(trim($rescart['prodsize'])!='NA' ? ' <b>Size</b>: '.trim($rescart['prodsize']) : '');
