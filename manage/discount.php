...
if($_POST['disccode']!=""){
	...
	$discstart = sqldate($_POST['discstart']);
	...
	if($discid>0){
		$sql="update ccd9discounts set disccode='$disccode', discamt='$discamt', disctype='$disctype', discstatus='$discstatus', discuse='$discuse', compid='$userid', discdescr='$discdescr', discexpiry='$discexpiry', discstart='$discstart', discemail='$discemail', disccur='$disccur', exclcats='".implode(",",$exclcats)."', inclcats='".implode(",",$inclcats)."', incltype='".implode(",",$incltype)."' where discid='$discid'";
		//echo $sql;
		//exit();
		$mysqli->query($sql);
		header("Location:discounts.php?msg=Discount code updated successfully!");
	}else{
		$sql = "insert into ccd9discounts (disccode, discamt, disctype, discstatus, discuse, compid, discdescr, discexpiry, discemail, disccur, exclcats, incltype, inclcats, discstart) values('".$disccode."', '".$discamt."', '".$disctype."', '".$discstatus."', '".$discuse."', '".$userid."', '".$discdescr."', '".$discexpiry."', '".$discemail."', '".$disccur."', '".implode(",",$exclcats)."', '".implode(",",$incltype)."', '".implode(",",$inclcats)."', '$discstart' )";
		$mysqli->query($sql);
	...
	if($row['discid']>0){
		...
		$discstart = inddate($row['discstart']);
    ...
    <div id="myTabContent" class="tab-content">
	  ...
	  <input type="hidden" name="delid">
		...
		<div class="form-group col-md-3">
			...
			<input type="text" name="discstart" value="<?php echo $discstart?>" maxlength="10" class="form-control datepicker">
        </div>
		<div class="form-group col-md-3">
