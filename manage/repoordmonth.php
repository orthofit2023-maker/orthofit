...
if($_GET['dl']=="xls" && checkuseraccess(0,0,5)){
	...
	header("Content-disposition:attachment;filename=repoordmonth.xls"); 
	header("Expires: 0");
...
?>
<div class="header">
	<h1 class="page-title">Sales by Month</h1>
	<ul class="breadcrumb">
	<li><a href="index.php">Home</a> </li>
	<li class="active">Sales by Month</li>
	</ul>
...
<div class="btn-toolbar list-toolbar">
	<form method="get" name="frm" action="repoordmonth.php">
...
<table class="table table-striped" <?php echo ($_GET['dl']=="xls" ? 'border="1"' : '')?>>
  ...
  <tbody>
	<?php 
	$i=0; 
	$sql="SELECT sum(ordtotal) as cnt, MONTH(orddate) as mon FROM `ccd9orders` o where o.status>0 ";
	...
	$sql=$sql." group by MONTH(orddate) order by MONTH(orddate)"; //cnt desc 
	...
	if ($num_rows>0){
		...
		?>
    <tr>
      <td><?php echo date('F',mktime(0,0,0,intval($row['mon']),'1',date("Y")))?></td>
