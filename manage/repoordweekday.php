...
if($_GET['dl']=="xls" && checkuseraccess(0,0,5)){
	...
	header("Content-disposition:attachment;filename=repoordweekday.xls"); 
	header("Expires: 0");
...
?>
<div class="header">
	<h1 class="page-title">Sales by Weekday</h1>
	<ul class="breadcrumb">
	<li><a href="index.php">Home</a> </li>
	<li class="active">Sales by Weekday</li>
	</ul>
...
<div class="btn-toolbar list-toolbar">
	<form method="get" name="frm" action="repoordweekday.php">
...
<table class="table table-striped" <?php echo ($_GET['dl']=="xls" ? 'border="1"' : '')?>>
  ...
  <tbody>
	<?php 
	$i=0;  $weekdyname = array('','Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday');
	$sql="SELECT sum(ordtotal) as cnt, DAYOFWEEK(orddate) as wday FROM `ccd9orders` o where o.status>0 ";
	...
	$sql=$sql." group by DAYOFWEEK(orddate) order by cnt desc ";
	...
	if ($num_rows>0){
		...
		?>
    <tr>
      <td><?php echo $weekdyname[intval($row['wday'])]?></td>
