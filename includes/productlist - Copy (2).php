...
$sqllist="SELECT t3.*, t3c.typename, t3c.typevalue,  c.*, IF(CURDATE() between c.offerfrdate and c.offertodate, '1', '0') as isoffer, IF(CURDATE() between c.discfrdate and c.disctodate, '1', '0') as isdiscount ".($_SESSION['compid']>0 ? ", w.wishid " : "").($opt=='sale' ? ", ct.status as salestatus" :"")." from ccd9products c join ccd9prod2type3 t3 on c.prodid=t3.prodid join ccd9types t3c on t3.typeid=t3c.typeid and t3c.opt=7  left join ccd9prod2cat t on t.prodid=c.prodid ".($type1>0 ? " left join ccd9prod2type1 v1 on v1.prodid=c.prodid " : "" ).($type2>0 ? " left join ccd9prod2type2 v2 on v2.prodid=c.prodid " : "").($_SESSION['compid']>0 ? " left join ccd9wishlist w on w.prodid=c.prodid and w.compid='".$_SESSION['compid']."'" : "").($opt=='sale' ? " left join ccd9cart ct on ct.prodid=c.prodid " : "" )." where 1=1 and c.prodstatus='1' ".($_SESSION['myCUR']=="US $" ? " and (c.zone='USD' or c.zone='ALL')" : " and (c.zone='INR' or c.zone='ALL')");
	...
	<div class="collection-header">
		<div class="collection-hero">
			...
			<div class="collection-hero__title-wrapper container">
				<h2 class="collection-hero__title"><?php echo $catname?></h2>
					...
					<div class="row">
						...
						if ($num_rows>0){ $i=0; 
							while($row=$result->fetch_array()){ 
								$produrl=getprodurl($row['produrl'],$opt);
								$arrphoto= array();
								$prodphoto=trim($row['photo']);
								//$prodgallery=trim($row['gallery']);
								$prodsize=$row['prodsize']; 
								$prodprice=$_SESSION['myCUR'].' '.$row['prodprice'];

								if(strstr($prodphoto,',')){
									$arrphoto=explode(',', trim($prodphoto));
									//$arrphoto[1]=trim($arrgallery[1]);
								}else{
									$arrphoto[0]=trim($prodphoto);
									$arrphoto[1]=trim($prodphoto);
								}
								$photoname = trim($row['produrl']).'-'.$row['tyepid'].'-'.strtolower(chr(65));
								...
								//-------------------------------------------------------------
								if(trim($arrphoto[1])!=''){
									$photoname = trim($row['produrl']).'-'.$row['tyepid'].'-'.strtolower(chr(66));
									$webpimg1 = $photoname.'.webp';

									$prodphoto=trim($arrphoto[1]);
									$photoext=trim(substr($prodphoto,strrpos($prodphoto,".")+1));
									$prodimg=$photoname.'.'.$photoext;
									if(!file_exists($imgpath.$prodimg)){
										$photocontent = file_get_contents($prodphoto);
									...
									if(!file_exists($imgpath.$webpimg1)){
										$image = imagecreatefromstring(file_get_contents($imgpath.$prodimg));
										ob_start();
										imagejpeg($image,NULL,100);
										$cont = ob_get_contents();
						...
						<div class="col-6 col-sm-6 col-md-4 col-lg-4 item">
							...
							<div class="product-image">
								...
								<a href="<?php echo $produrl?>" class="product-img">
									<!-- image -->
									<img class="primary blur-up lazyload" data-src="collection/<?php echo $webpimg?>" src="collection/<?php echo $webpimg?>" alt="image" title="">
									<!-- End image -->
									<!-- Hover image -->
									<img class="hover blur-up lazyload" data-src="collection/<?php echo $webpimg1?>" src="collection/<?php echo $webpimg1?>" alt="image" title="">
									<!-- End hover image -->
								</a>
								<!--End Product Image-->
								...
								<div class="button-set style0 d-none d-md-block">
									<ul>
										...
										<!--Wishlist Button-->
										<li><a class="btn-icon wishlist add-to-wishlist" href="wishlist"><i class="icon an an-heart-l"></i> <span class="tooltip-label top">Add To Wishlist</span></a></li>
										<!--End Wishlist Button-->
										<!--Compare Button-->
