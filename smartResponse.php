...
//echo 'xxxxxxxxxxxxx'.$_POST['ordref'];
//print_r($_POST);
//print_r($_GET);
if(isset($_POST['order_id'])){

$rsdata= query_first("select compid, ordref, orderid from ccd9orders where ordref='".$_POST['order_id']."' and status='0'");
if ($rsdata['compid']>0){
$compid=$rsdata['compid'];
$orderid=$rsdata['orderid'];
$ordref=$rsdata['ordref'];



$statusURL=$statusURL.$rsdata['ordref'];
$headerValue =  base64_encode($apikey);
$header = array();
$header[] = 'Content-type: application/x-www-form-urlencoded';
$header[] = 'Authorization: Basic '.$headerValue;		
$header[] = 'x-merchantid: '.$merchantid;
$header[] = 'x-customerid: '.$compid;		
...
if ($curlerr != ''){
	...
	if($res->status=='NEW'){
		//exit("order?errmsg=Please complete your payment");
		header("location:order?errmsg=Please complete your payment");
	}else{
		/*
		echo '<br><br>--------------------------------';
		echo $res->status; 
		...
		*/
		...
		if($order_status==="AUTHORIZING" || $order_status==="AUTHORIZED" || $order_status==="CHARGED" ){ 
			...
			if($row['ordtotal']!=$rettotal || ($prodtot-$row['discountamt']+$row['shippingamt'])!=$rettotal){
				...
				$orderemail=$orderemail."<br> ordref ".$ordref;

				sendsmtpmail($to,$subject,$orderemail);
				$ordermsg="Transaction could not be completed now! Kindly contact our office directly on  84549 20321 for further assistance";
				header("location:order?errmsg=".$ordermsg);
				exit();
				//exit("order?errmsg=$subject");
			}
			...
			$mysqli->query("update ccd9cart set orderid='$orderid', status='1' where sessionid='".session_id()."' and compid='".$_SESSION["compid"]."' and ordref='$ordref' and prodqty>0 and status='0'");
	...
	header("location:".$serverurl."order?errmsg=Please complete your payment");
	exit("order?errmsg=Please complete your payment");
}
}else{
	header("location:".$serverurl."order?errmsg=Please complete your payment");
